              ...
              {myListings.map(item => (
                <div key={item.id} className="card" style={{ padding: '1.25rem' }}>
                  <div style={{ height: '160px', borderRadius: '8px', overflow: 'hidden', marginBottom: '0.75rem', background: '#F1F5F9' }}>
                    <img src={item.image || (item.type === 'rent' ? FALLBACK_IMAGES.cinemaCamera : FALLBACK_IMAGES.leatherBag)} alt={item.title} onError={(e) => { e.target.onerror = null; e.target.src = item.type === 'rent' ? FALLBACK_IMAGES.cinemaCamera : FALLBACK_IMAGES.leatherBag; }} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
